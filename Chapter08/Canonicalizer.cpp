...
ir::Statements Canonicalizer::traceSchedule(BasicBlocks &&blocks) const {
  ...
  // make sure result ends with a jump
  match(res.back())([](const ir::Jump &) {}, [](const ir::ConditionalJump &) {},
                    [&](const auto &) {
                      res.emplace_back(ir::Jump{m_tempMap.namedLabel("end")});
                    });
