...
auto checkMemberAddress(OptReg &r, int index) {
  return checkBinaryOperation(
    ir::BinOp::PLUS, checkReg(r),
    checkBinaryOperation(ir::BinOp::MUL, checkInt(index), checkInt(WORD_SIZE)));
}
...
auto checkProgram(CheckStatements &&... checkStatements) {
  return
    [checkStatements = std::make_tuple(
       checkLabel(start), std::forward<CheckStatements>(checkStatements)...)](
...
in
  ...
  OptReg r;
  checkProgram(checkMove( // move result of malloc(array_size) to r
                 checkReg(r), checkExternalCall("malloc", checkBinaryOperation(
                                                            ir::BinOp::MUL,
                                                            checkInt(WORD_SIZE),
                                                            checkInt(2)))),
          ...
          checkMove( // move result of malloc(array_size) to
            ...
            checkExternalCall(
              "malloc", checkBinaryOperation(
                          ir::BinOp::MUL, checkInt(WORD_SIZE), checkInt(2)))),
...
in
  if 1 then a := 3
    ...
    checkProgram(
      ...
      checkMove(            // return 0
        checkReg(rv), checkInt(0)),
      checkJump(checkLabel(end)), checkLabel(trueDest),
...
in
  ...
  checkProgram(
    ...
    checkConditionalJump( // skip the loop if a > limit
      ir::RelOp::GT, checkReg(a), checkReg(limit), &loopDone, &loopStart),
    checkLabel(loopStart),
...
TEST_CASE("break") {
  SECTION("for") {
    ...
    checkProgram(
      ...
      checkConditionalJump( // skip the loop if a > limit
        ir::RelOp::GT, checkReg(a), checkReg(limit), &loopDone, &loopStart),
      checkLabel(loopStart), checkLabel(loopDone),
  ...
  SECTION("while") {
    ...
    checkProgram(checkLabel(loopStart),
                 checkConditionalJump( // test
                   ir::RelOp::EQ, checkInt(1), checkInt(0), &loopDone,
                   &loopNotDone // if test equals 0 jumps to loopDone
                   ),
                 checkLabel(loopNotDone), checkLabel(loopDone),
                 checkMove(checkReg(rv), checkInt(0)),
                 checkJump(checkLabel(end)), checkLabel(afterBreak),
                 checkJump( // jump back to loop Start
  ...
  intlist{hd = 3, tl = intlist{hd = 4, tl = nil}}
      ...
      checkProgram(
        checkMove( // move result of malloc(record_size) to r1
          checkReg(r1), checkExternalCall("malloc", checkInt(2 * WORD_SIZE))),
        ...
        checkMove( // move result of malloc(record_size)
                   // to r3
          checkReg(r3), checkExternalCall("malloc", checkInt(2 * WORD_SIZE))),
        checkMove( // init first member
...
in
  tree{key = 1, children = treelist{hd = tree{key = 2, children = nil}, tl = nil}}
      ...
      checkProgram(
        ...
        checkMove( // move result of malloc(record_size)
                   // to r2
          checkReg(r2), checkExternalCall("malloc", checkInt(2 * WORD_SIZE))),
...
in
      ...
      checkProgram(
        ...
        checkConditionalJump(      // n = 0
          ir::RelOp::EQ, checkReg(n), checkInt(0), &trueDest[0], &falseDest[0]),
        checkLabel( // else
          falseDest[0]),
        ...
        checkConditionalJump( // n
                              // =
                              // 1
          ir::RelOp::EQ, checkReg(n), checkInt(1), &trueDest[1], &falseDest[1]),
        ...
        checkMove( // fib(n-1)
          checkReg(r[3]),
          checkCall(
            checkLabel(functionLabel), checkStaticLink<2>(),
            checkBinaryOperation(ir::BinOp::MINUS, checkReg(n), checkInt(1)))),
        checkMove(checkReg(r[5]), checkReg(r[3])),
        checkMove( // fib(n-2)
          checkReg(r[4]),
          checkCall(
            checkLabel(functionLabel), checkStaticLink<2>(),
            checkBinaryOperation(ir::BinOp::MINUS, checkReg(n), checkInt(2)))),
        checkMove( // add two previous calls
          checkReg(r[2]), checkBinaryOperation(ir::BinOp::PLUS, checkReg(r[5]),
                                               checkReg(r[4]))),
                  ...
                  checkMemoryAccess( // i is
                    ...
                    checkBinaryOperation(
                      ...
                      checkMemoryAccess(checkBinaryOperation(
                        ir::BinOp::PLUS, checkReg(fp), checkInt(-WORD_SIZE))),
