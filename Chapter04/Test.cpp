...
auto checkIdentifier(const std::string &name) {
  return [name](const ast::Identifier &id) { REQUIRE(id.name == name); };
}
...
template <typename CheckExp> auto checkVarSubscript(CheckExp &&checkExp) {
...
auto checkNil() {
  return
    [](const ast::Expression &ast) { REQUIRE(get<ast::NilExpression>(&ast)); };
}
...
template <typename... CheckExps> auto checkSequence(CheckExps &&... checkExps) {
  return [&](const ast::Expression &ast) {
    auto pSequence = get<ast::ExpressionSequence>(&ast);
    REQUIRE(pSequence);
    applyFunctionsToVector(pSequence->exps,
                           std::forward<CheckExps>(checkExps)...);
  };
}
...
TEST_CASE("sequence") {
  ...
  SECTION("multiple expressions") {
    ...
    checkSequence(checkLValue("i"), checkLValue("j"));
  }
}
...
auto checkArithmetic(CheckLhs &&checkLhs, CheckOps &&... checkOps) {
  return [&](const ast::Expression &ast) {
    ...
    applyFunctionsToVector(pArithmetic->rest,
                           std::forward<CheckOps>(checkOps)...);
  };
}
...
TEST_CASE("integer") {
  ...
  SECTION("negative") {
    REQUIRE(parse("-42", ast));
    // -x is parsed as 0-x
    checkArithmetic(checkInt(0),
                    checkOperation(ast::Operation::MINUS, checkInt(42)))(ast);
  }
}
...
TEST_CASE("arithmetic") {
  ...
  SECTION("simple") {
    ...
    checkArithmetic(checkInt(2),
                    checkOperation(ast::Operation::PLUS, checkInt(3)))(ast);
    REQUIRE(parse("2-3", ast));
    checkArithmetic(checkInt(2),
                    checkOperation(ast::Operation::MINUS, checkInt(3)))(ast);
    REQUIRE(parse("2*3", ast));
    checkArithmetic(checkInt(2),
                    checkOperation(ast::Operation::TIMES, checkInt(3)))(ast);
    REQUIRE(parse("2/3", ast));
    checkArithmetic(checkInt(2),
                    checkOperation(ast::Operation::DIVIDE, checkInt(3)))(ast);
  }
  SECTION("precedence") {
    REQUIRE(parse("2*3+5-1/24", ast));
    checkArithmetic(
      checkArithmetic(checkInt(2),
                      checkOperation(ast::Operation::TIMES, checkInt(3))),
      checkOperation(ast::Operation::PLUS, checkInt(5)),
      checkOperation(
        ast::Operation::MINUS,
        checkArithmetic(checkInt(1), checkOperation(ast::Operation::DIVIDE,
                                                    checkInt(24)))))(ast);
  }
  SECTION("with subexpression") {
    ...
    checkArithmetic(
      ...
      checkOperation(
        ast::Operation::PLUS, [&](const ast::Expression & /* ast */) {
          checkArithmetic(
            [](const ast::Expression &ast) {
              REQUIRE(get<ast::VarExpression>(&ast));
            },
            checkOperation(
              ast::Operation::DIVIDE, [&](const ast::Expression &ast) {
                auto pSequence = get<ast::ExpressionSequence>(&ast);
                REQUIRE(pSequence);
...
TEST_CASE("comparison") {
  ...
  SECTION("simple") {
    REQUIRE(parse("2=3", ast));
    checkArithmetic(checkInt(2),
                    checkOperation(ast::Operation::EQUAL, checkInt(3)))(ast);
    REQUIRE(parse("2<>3", ast));
    checkArithmetic(
      checkInt(2), checkOperation(ast::Operation::NOT_EQUAL, checkInt(3)))(ast);
    REQUIRE(parse("2>3", ast));
    checkArithmetic(checkInt(2), checkOperation(ast::Operation::GREATER_THEN,
                                                checkInt(3)))(ast);
    REQUIRE(parse("2<3", ast));
    checkArithmetic(
      checkInt(2), checkOperation(ast::Operation::LESS_THEN, checkInt(3)))(ast);
    REQUIRE(parse("2>=3", ast));
    checkArithmetic(checkInt(2), checkOperation(ast::Operation::GREATER_EQUAL,
                                                checkInt(3)))(ast);
    REQUIRE(parse("2<=3", ast));
    checkArithmetic(checkInt(2), checkOperation(ast::Operation::LESS_EQUAL,
                                                checkInt(3)))(ast);
  }
  SECTION("precedence") {
    ...
    checkArithmetic(
      checkArithmetic(checkInt(2),
                      checkOperation(ast::Operation::TIMES, checkInt(5))),
      checkOperation(
        ast::Operation::LESS_THEN,
        checkArithmetic(checkInt(3), checkOperation(ast::Operation::PLUS,
                                                    checkInt(4)))))(ast);
  ...
  SECTION("with subexpression") {
    ...
    checkArithmetic(
      ...
      checkOperation(ast::Operation::EQUAL, [](const ast::Expression &ast) {
        auto pSequence = get<ast::ExpressionSequence>(&ast);
        REQUIRE(pSequence);
        REQUIRE(pSequence->exps.size() == 1);
        checkArithmetic(checkInt(4),
                        checkOperation(ast::Operation::GREATER_EQUAL,
                                       checkInt(2)))(pSequence->exps[0]);
      }))(ast);
  }
}

template <typename CheckTest, typename CheckThen,
          typename CheckElse = decltype(checkNil())>
auto checkIf(CheckTest &&checkTest, CheckThen &&checkThen,
             CheckElse &&checkElse = checkNil()) {
  return [=](const ast::Expression &ast) {
...
auto checkAnd(CheckLhs &&checkLhs, CheckRhs &&checkRhs) {
  return checkIf(std::forward<decltype(checkLhs)>(checkLhs),
                 std::forward<decltype(checkRhs)>(checkRhs), checkInt(0));
}

// a|b is translated to if a then 1 else b
template <typename CheckLhs, typename CheckRhs>
auto checkOr(CheckLhs &&checkLhs, CheckRhs &&checkRhs) {
  return checkIf(std::forward<decltype(checkLhs)>(checkLhs), checkInt(1),
                 std::forward<decltype(checkRhs)>(checkRhs));
}
...
TEST_CASE("boolean") {
  ...
  SECTION("compound") {
    ...
    checkAnd(checkInt(2),
             checkSequence(checkOr(
               checkInt(3), checkSequence(checkArithmetic(
                              checkInt(2), checkOperation(ast::Operation::MINUS,
                                                          checkInt(4)))))))(
      ast);
  }
  SECTION("with subexpression") {
    ...
    checkAnd(
      checkFunctionCall("f", checkInt(2)),
      checkSequence(checkOr(
        checkArithmetic(checkInt(3),
                        checkOperation(ast::Operation::EQUAL, checkInt(4))),
        checkArithmetic(checkInt(2), checkOperation(ast::Operation::DIVIDE,
                                                    checkLValue("b"))))))(ast);
  }
}
...
auto checkRecord(const std::string &type, CheckFields &&... checkFields) {
  return [&, type](const ast::Expression &ast) {
    ...
    applyFunctionsToVector(pRecord->fields,
                           std::forward<CheckFields>(checkFields)...);
  };
}
...
TEST_CASE("record") {
  ...
  SECTION("not empty") {
    ...
    checkRecord("tt", checkRecordField("a", checkInt(3)),
                checkRecordField("b", checkLValue("a")))(ast);
  }
  SECTION("with subexpression") {
    ...
    checkRecord(
      ...
      checkRecordField(
        "a",
        checkFunctionCall(
          "f", checkArithmetic(checkInt(2), checkOperation(ast::Operation::PLUS,
                                                           checkInt(3))))),
      checkRecordField(
        "b", checkSequence(checkAnd(
               checkLValue("a"),
               checkSequence(checkArithmetic(
                 checkLValue("b"),
                 checkOperation(ast::Operation::DIVIDE, checkInt(2))))))))(ast);
  }
}

template <typename CheckSize, typename CheckInit>
auto checkArray(const std::string &type, CheckSize &&checkSize,
                CheckInit &&checkInit) {
  return [=](const ast::Expression &ast) {
...
TEST_CASE("array") {
  ...
  SECTION("simple") {
    ...
    checkArray("a", checkInt(2), checkInt(3))(ast);
  }
  SECTION("with subexpression") {
...
TEST_CASE("assignment") {
  ...
  SECTION("simple") {
    ...
    checkAssignment(checkLValue("a"), checkInt(3))(ast);
  }
  SECTION("with subexpression") {
    REQUIRE(parse("a[b.d] := f(5/d)", ast));
    checkAssignment(
      checkLValue("a", checkVarSubscript(checkLValue("b", checkVarField("d")))),
      checkFunctionCall(
        "f", checkArithmetic(checkInt(5), checkOperation(ast::Operation::DIVIDE,
                                                         checkLValue("d")))))(
      ast);
  }
}
...
TEST_CASE("if then") {
  ...
  SECTION("no else") {
    ...
    checkIf(checkLValue("a"),
            checkAssignment(checkLValue("a"), checkInt(3)))(ast);
    SECTION("with subexpression") {
      REQUIRE(parse("if f(b)=2/d then a[2] := g(b.d)", ast));
      checkIf(
        checkArithmetic(
          checkFunctionCall("f", checkLValue("b")),
          checkOperation(
            ast::Operation::EQUAL,
            checkArithmetic(checkInt(2), checkOperation(ast::Operation::DIVIDE,
                                                        checkLValue("d"))))),
        checkAssignment(
          ...
          checkFunctionCall("g", checkLValue("b", checkVarField("d")))))(ast);
    }
    SECTION("nested") {
      REQUIRE(parse("if 2>3 then if 2<3 then a := 3", ast));
      checkIf(checkArithmetic(
                checkInt(2),
      ...
      checkIf(
        checkArithmetic(
          checkInt(2),
          checkOperation(ast::Operation::GREATER_THEN, checkInt(3))),
        checkSequence(checkIf(
          checkArithmetic(checkInt(2), checkOperation(ast::Operation::LESS_THEN,
                                                      checkInt(3))),
          checkAssignment(checkLValue("a"), checkInt(3)))))(ast);
    }
  }
  SECTION("with else") {
    ...
    checkIf(checkLValue("a"), checkAssignment(checkLValue("a"), checkInt(3)),
            checkAssignment(checkLValue("a"), checkInt(2)))(ast);
    REQUIRE(parse("if a then 5 else 7", ast));
    checkIf(checkLValue("a"), checkInt(5), checkInt(7))(ast);
    SECTION("with subexpression") {
      ...
      checkIf(
        ...
        checkArithmetic(
          ...
          checkOperation(ast::Operation::MINUS, checkLValue("b"))))(ast);
    }
    SECTION("nested") {
      ...
      checkIf(
        ...
        checkSequence(checkIf(
          checkArithmetic(checkInt(2), checkOperation(ast::Operation::LESS_THEN,
                                                      checkInt(3))),
          checkAssignment(checkLValue("a"), checkInt(4)))),
        checkAssignment(checkLValue("b"), checkInt(4)))(ast);
      REQUIRE(parse("if 2>3 then (if 2<3 then a := 4 else b := 4)", ast));
      checkIf(
        checkArithmetic(
          checkInt(2),
...
TEST_CASE("while") {
  ...
  checkWhile(
    checkArithmetic(checkLValue("b"),
                    checkOperation(ast::Operation::GREATER_THEN, checkInt(0))),
    checkSequence(
      checkAssignment(checkLValue("a"), checkInt(2)),
      checkAssignment(
        checkLValue("b"),
        checkArithmetic(checkLValue("b"), checkOperation(ast::Operation::MINUS,
                                                         checkInt(1))))))(ast);
}

template <typename CheckLo, typename CheckHi, typename CheckBody>
auto checkFor(const std::string var, CheckLo &&checkLo, CheckHi &&checkHi,
              CheckBody &&checkBody) {
  return [=](const ast::Expression &ast) {
...
TEST_CASE("for") {
  ...
  checkFor("a", checkInt(0), checkInt(4),
           checkFunctionCall("f", checkLValue("a")))(ast);
}
  ...
  SECTION("in a loop") {
    ...
    checkWhile(
      checkArithmetic(
        ...
        checkOperation(ast::Operation::GREATER_THEN, checkInt(0))),
      checkSequence(
        checkIf(checkSequence(checkLValue("x")), checkBreak()),
        checkAssignment(checkLValue("a"), checkInt(2)),
        checkAssignment(checkLValue("b"),
                        checkArithmetic(checkLValue("b"),
...
auto checkLet(const std::tuple<CheckDecls...> &checkDecls,
              const std::tuple<CheckExps...> &checkExps) {
  return [=](const ast::Expression &ast) {
...
auto checkFunctionDeclarations(CheckFuncDecls &&... checkFuncDecls) {
  return [&](const ast::Declaration &decl) {
    ...
    applyFunctionsToVector(*pFunctionDeclarations,
                           std::forward<CheckFuncDecls>(checkFuncDecls)...);
  };
}

template <typename CheckInit>
auto checkVarDeclaration(const std::string &name, CheckInit &&checkInit,
...
auto checkTypeDeclarations(CheckTypeDecls &&... checkTypeDecls) {
  return [&](const ast::Declaration &decl) {
    ...
    applyFunctionsToVector(*pTypeDeclarations,
                           std::forward<CheckTypeDecls>(checkTypeDecls)...);
  };
}

template <typename CheckBody, typename... CheckParams>
auto checkFunctionDeclaration(const std::string &name,
  ...
  return [&, result](const ast::FunctionDeclaration &funcDecl) {
    ...
    applyFunctionsToVector(funcDecl.params,
                           std::forward<CheckParams>(checkParams)...);
    checkBody(funcDecl.body);
  };
}

template <typename CheckBody, typename... CheckParams>
auto checkUntypedFunctionDeclaration(const std::string &name,
                                     CheckBody &&checkBody,
                                     CheckParams &&... checkParams) {
  return checkFunctionDeclaration(name, boost::none,
                                  std::forward<CheckBody>(checkBody),
                                  std::forward<CheckParams>(checkParams)...);
}
...
auto checkRecordType(CheckFields &&... checkFields) {
  return [&](const ast::Type &type) {
    ...
    REQUIRE(pRecordType);
    applyFunctionsToVector(pRecordType->record,
                           std::forward<CheckFields>(checkFields)...);
  };
}
    ...
    std::make_tuple(
      checkTypeDeclarations(
        ...
        checkTypeDeclaration(
          "t2", checkRecordType(checkField("a", "int"), checkField("b", "t1"))),
        checkTypeDeclaration("t3", checkArrayType("t2"))),
      checkVarDeclaration("a", checkInt(2)),
      checkVarDeclaration("b", checkNil(), "t3"s),
      checkVarDeclaration(
        "c", checkRecord("t2", checkRecordField("a", checkInt(2))), "t2"s),
      checkFunctionDeclarations(
        checkUntypedFunctionDeclaration(
          "f",
          checkAssignment(checkLValue("b", checkVarSubscript(checkInt(2))),
                          checkLValue("i")),
          checkField("i", "int"), checkField("b", "t3")),
        checkFunctionDeclaration("g", "t2"s, checkInt(4)))),
    std::make_tuple(
      checkFunctionCall("f",
                        checkArithmetic(checkLValue("a"),
                                        checkOperation(ast::Operation::PLUS,
                                                       checkFunctionCall("g"))),
                        checkLValue("b")),
      checkAssignment(checkLValue("c", checkVarField("a")), checkLValue("a"))))(
    ast);
}
