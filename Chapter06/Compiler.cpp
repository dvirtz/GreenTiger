...
Compiler::result_type Compiler::compile(const ast::Expression &ast) {
  return compile(m_translator.outermost(), ast);
}
...
Compiler::result_type Compiler::compile(Translator::Level level,
  ...
  for (const auto &arg : exp.args) {
    auto argType = compile(level, arg);
    if (!argType) {
      return {};
    }
...
Compiler::result_type Compiler::compile(Translator::Level level,
                                        const ast::ArithmeticExpression &exp) {
  auto lhs = compile(level, exp.first);
  if (!lhs) {
    return {};
  }
  ...
  if (!exp.rest.empty() && !checkType(exp.rest[0].op, *lhs, id(exp.first))) {
    return {};
  }

  for (const auto &opExp : exp.rest) {
    auto rhs = compile(level, opExp.exp);
    if (!rhs) {
      return rhs;
    }
...
Compiler::result_type Compiler::compile(Translator::Level level,
                                        const ast::RecordExpression &exp) {
  ...
  for (const auto &field : exp.fields) {
    auto fieldType = compile(level, field.exp);
    if (!fieldType) {
      return {};
    }
...
Compiler::result_type Compiler::compile(Translator::Level level,
                                        const ast::AssignExpression &exp) {
  auto varType = compile(level, exp.var);
  if (!varType) {
    return {};
  }

  auto expType = compile(level, exp.exp);
  if (!expType) {
    return {};
  }
...
Compiler::result_type Compiler::compile(Translator::Level level,
  ...
  auto test = compile(level, exp.test);
  if (!test) {
    return {};
  }
  ...
  auto thenExp = compile(level, exp.thenExp);
  if (!thenExp) {
    return {};
  }

  if (exp.elseExp) {
    auto elseExp = compile(level, *exp.elseExp);
    if (!elseExp) {
      return {};
    }
    ...
    if (!equalTypes(*thenExp, *elseExp)) {
      ...
      // expression is converted to an if and the second expression is generated
      // by the parser
      if (hasType<IntType>(*thenExp)) {
        return m_errorHandler(id(*exp.elseExp),
                              "Expression must have int type");
      }
    ...
    if (!hasType<VoidType>(*thenExp)) {
      return m_errorHandler(id(exp.thenExp),
                            "Expression must produce no value");
    }
  }
...
Compiler::result_type Compiler::compile(Translator::Level level,
  ...
  auto body = compile(level, exp.body);
  if (!body) {
    return {};
  }
...
Compiler::result_type Compiler::compile(Translator::Level /* level */,
  ...
  if (!m_withinLoop) {
    return m_errorHandler(id(exp),
                          "break is only legal within a while or a for loop");
  };
...
Compiler::result_type Compiler::compile(Translator::Level level,
                                        const ast::ForExpression &exp) {
  auto fromExp = compile(level, exp.lo);
  if (!fromExp) {
    return {};
  }
  ...
  auto toExp = compile(level, exp.hi);
  if (!toExp) {
    return {};
  }
  ...
  auto bodyExp = compile(level, exp.body);
...
Compiler::result_type Compiler::compile(Translator::Level level,
  ...
  auto sizeExp = compile(level, exp.size);
  if (!sizeExp) {
    return {};
  }
  ...
  auto initExp = compile(level, exp.init);
  if (!initExp) {
    return {};
  }
...
Compiler::result_type Compiler::compile(Translator::Level level,
                                        const ast::ExpressionSequence &exp) {
  CompiledExpression res{s_voidType};

  for (const auto &e : exp.exps) {
    auto compiled = compile(level, e);
    if (!compiled) {
      return {};
    }
...
bool Compiler::addToEnv(Translator::Level level,
  ...
  for (const auto &dec : decs) {
    ...
    if (dec.result) {
      ...
      if (!type) {
        return m_errorHandler(dec.name.id,
                              "Undeclared type " + dec.result->name);
      }
    ...
    for (const auto &param : dec.params) {
      ...
      if (!type) {
        return m_errorHandler(dec.name.id,
                              "Undeclared type " + param.type.name);
      }
    ...
    function.m_level = m_translator.newLevel(level, function.m_label, formals);
  ...
  for (const auto &dec : decs) {
    ...
    auto compiled = compile(funcType->m_level, dec.body);
    if (!compiled) {
      return false;
    }
...
bool Compiler::addToEnv(Translator::Level level,
  ...
  auto compiled = compile(level, dec.init);
  if (!compiled) {
    return false;
  }
...
void Compiler::addToEnv(const ValueMap::key_type &name,
