    ...
    checkProgram(program,
                 checkMove( // move 2 to b
                     ...
                     checkMove(checkReg(regs[2]),
                               checkImm(1)) > // move 1 to r
                     checkMove(checkReg(regs[4]), checkImm(0)) >
                     checkConditionalJump(ir::RelOp::GT, regs[0], regs[4], trueDest) > // check condition,
                                                                                       // jumping to trueDest
                                                                                       // or falseDest
                                                                                       // accordingly
                     ...
                     checkLabel(trueDest) > ':' >
                     checkMove(checkReg(regs[6]), checkImm(0)) >
                     checkConditionalJump(                            // test
                         ir::RelOp::EQ, regs[2], regs[6], loopDone) > // if test equals 0 jumps to loopDone
                     checkLabel(loopNotDone) > ':' >
                     checkMove(checkReg(regs[8]), checkImm(1)) >
                     checkBinaryOperation(ir::BinOp::PLUS, checkReg(regs[1]), checkReg(regs[8]), regs[9]) >
                     checkMove( // a := a + 1
