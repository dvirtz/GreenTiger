...
TEST_CASE("break") {
  SECTION("for") {
    ...
    checkProgram(
      ...
      checkMove( // move 0 to a
        ...
        > checkMove(returnReg(), checkImm(0)) > checkJump(end)
        > checkLabel(afterBreak) > ':'
        > checkMove(checkReg(regs[2]), checkImm(1))
        > checkBinaryOperation(ir::BinOp::PLUS, checkReg(regs[0]),
                               checkReg(regs[2]), regs[3])
        > checkMove( // a := a + 1
  ...
  SECTION("while") {
    ...
    checkProgram(
      program,
      checkLabel(loopStart) > ':' > checkMove(checkReg(regs[0]), checkImm(1))
        > checkMove(checkReg(regs[1]), checkImm(0))
        > checkConditionalJump( // test
            ir::RelOp::EQ, regs[0], regs[1], loopDone)
        > // if test equals 0 jumps to loopDone
        checkLabel(loopNotDone) > ':' > checkLabel(loopDone) > ':'
        > checkMove(returnReg(), checkImm(0)) > checkJump(end)
        > checkLabel(afterBreak) > ':' > checkJump( // jump back to loop Start
                                           loopStart)
        > checkLabel(end) > ':',
      x3::eps, x3::eps, false);
  }
