...
inline auto checkImm(int i)
{
  ...
  auto const r = x3::rule<struct imm>{"imm"} = (x3::eps(arch == "x64") | x3::lit("#")) >> checkImm(i);
  return r;
}
                                 ...
                                 const CheckRight &right, OptReg &result)
{
  auto const r = x3::rule<struct binop>{"binop"} =
      (x3::eps(arch == "x64" && op == ir::BinOp::DIV) >
       checkMove(x3::lit("EAX"), left) > x3::lit(binOp(op)) > right > checkMove(checkReg(result), x3::lit("EAX"))) |
...
inline auto checkMemoryAccess(const CheckExp &checkExp)
{
  auto const r = x3::rule<struct memory_access>{"memory access"} =
      (arch == "m68k" ? x3::lit('(') : x3::lit('[')) > checkExp >> (arch == "m68k" ? ')' : ']');
  return r;
}
...
inline auto checkArg(int index, const CheckExp &checkExp)
{
  ...
  auto const r = x3::rule<struct arg>("argument") =
      (x3::eps(arch == "m68k") > checkMove(x3::lit('-') > checkMemoryAccess(x3::lit("sp")), checkExp)) |
      (x3::eps(arch == "x64") > ((x3::eps(index < 4) > checkMove(x3::lit(argReg(index)), checkExp)) | (x3::eps(index >= 4) > "push" > checkExp)));
  return r;
}
...
inline auto checkCall(const CheckLabel &checkLabel, const CheckArgs &checkArgs)
{
  auto const r = x3::rule<struct call>{"call"} =
      checkArgs >> ((arch == "m68k") ? x3::lit("JSR") : x3::lit("call")) > checkLabel;
  return r;
}
                           ...
                           const CheckPrepareArgs &checkPrepareArgs = x3::eps)
{
  auto const r = x3::rule<struct local_call>{"local call"} =
      checkStaticLink(gsl::span<OptReg, 1>{&staticLink, 1}, temps) > checkCall(checkLabel, checkPrepareArgs > checkArg(0, checkReg(staticLink)) > checkArgs);
  return r;
}
...
inline auto checkConditionalJump(ir::RelOp op, OptReg &left, OptReg &right, OptLabel &destination)
{
  auto const r = x3::rule<struct conditional_jump>{"conditional jump"} =
      ...
      ((x3::eps(arch == "m68k") > checkReg(left) > ',' > checkReg(right)) |
       (x3::eps(arch == "x64") > checkReg(right) > ',' > checkReg(left))) > x3::lit(conditionalJump(op)) > checkLabel(destination);
  return r;
}
...
inline auto checkParameterAcess(int index, const CheckBase &checkBase, gsl::span<OptReg, 2> temps, OptReg &result, bool escapes = false)
{
  auto const r = x3::rule<struct parameter_access>{"parameter access"} =
