...
in
    ...
    checkProgram(program, checkLabel(functionLabel), ':', // function label
                 ...
                 checkMove(returnReg(), checkImm(0)), branchToEnd(end));
  }
...
in
      ...
      checkProgram(
        ...
        checkLabel(joinDest[0]), ':', checkMove(checkReg(regs[4]), checkImm(0)),
        ...
        checkJump(functionEnd),
        checkLabel( // then
          trueDest[0]),
        ':', checkMove(checkReg(regs[3]), checkImm(1)), checkJump(joinDest[0]),
        checkLabel( // then
          trueDest[2]),
        ':',
        checkMove(checkReg(regs[17]), checkInlineParameterAccess(0, regs[1])),
        checkJump(joinDest[2]), checkLabel(functionEnd), ':', checkMain(),
        checkMove(returnReg(), checkImm(0)), branchToEnd(end));
    }
...
in
      ...
      checkProgram(
        program, checkLabel(fStart), ':',
        checkStaticLink(staticLinks[0], temps[0]),
        checkCall(checkLabel(gStart), checkArg(0, checkReg(staticLinks[0][0]))
                                        > checkArg(1, checkImm(1))),
        branchToEnd(fEnd), checkLabel(gStart), ':',
        checkStaticLink(staticLinks[1], temps[1]),
        checkCall(checkLabel(hStart), checkArg(0, checkReg(staticLinks[1][0]))
                                        > checkArg(1, checkImm(2))),
        branchToEnd(gEnd), checkLabel(hStart), ':',
        checkStaticLink(staticLinks[2], temps[2]),
        checkCall(checkLabel(fStart), checkArg(0, checkReg(staticLinks[2][0]))
                                        > checkArg(1, checkImm(3))),
        branchToEnd(hEnd), checkMain(), checkMove(returnReg(), checkImm(0)),
        branchToEnd(end));
    }
  }
...
in
 f(3)
    ...
    checkProgram(
      program, checkLabel(g), ':',
      // i is fetched from f's frame
      checkMove(checkReg(regs[0]), checkImm(-wordSize())),
      checkBinaryOperation(ir::BinOp::PLUS, framePointer(), checkReg(regs[0]),
                           regs[1]),
