...
std::vector<temp::Register> CallingConvention::callDefinedRegisters() const {
  return {reg(Registers::RAX), reg(Registers::RCX), reg(Registers::RDX),
          reg(Registers::R8),  reg(Registers::R9),  reg(Registers::R10),
          reg(Registers::R11)};
}

assembly::Registers CallingConvention::calleeSavedRegisters() const {
  return {reg(Registers::RBX), reg(Registers::RBP), reg(Registers::RDI),
...
assembly::Registers CallingConvention::argumentRegisters() const {
  return {reg(Registers::RCX), reg(Registers::RDX), reg(Registers::R8),
          reg(Registers::R9)};
}
