...
struct DagMatcher {
  ...
  struct MatchData {
    ...
    void move(MatchData &&other) {
      assert(other.m_sources.empty()
             && "Sources should have already been used");
      assert(other.m_immediates.empty()
             && "Immediates should have already been used");
  ...
  bool match(const ir::ConditionalJump &code,
             const ir::ConditionalJump &pattern, MatchData &matchData) const;
    ...
    helpers::match(dag)(
      [&](const ir::Expression &exp) {
        helpers::match(exp)(
          ...
          [&](const ir::Call &) {
            matchData.m_destinations.push_back(
...
bool DagMatcher::match(const ir::ConditionalJump &code,
...
bool DagMatcher::match(const ir::Call &code, const ir::Call &pattern,
  ...
  ranges::move(m_codeGenerator.translateArgs(args, m_tempMap),
               ranges::back_inserter(matchData.m_instructions));
  ranges::push_back(matchData.m_destinations,
                    m_codeGenerator.m_callingConvention.callDefinedRegisters());
  return true;
} // namespace assembly
