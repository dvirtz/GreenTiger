...
static std::string tokenName(TokenIds tokenId)
{
  static const std::string tokenNames[] = {
    ...
    "VAR", "TYPE", "COM_START", "COM_END", "ANY"
  };
...
public:
  TigerLexer() {
    ...
    this->self
      = m_initStartComment[initStartComment]
      | m_initEndComment[initEndComment]
      ;

    for (const auto& punctuation : punctuations)
    {
      this->self.define(lex::token_def<>(punctuation.first, punctuation.second)[print]);
    }

    for (const auto& reservedWord : reservedWords)
    {
      this->self.define(lex::token_def<>(reservedWord.first, reservedWord.second)[print]);
    }
    ...
    this->self("COMMENT")
      = m_commentStartComment[commentStartComment]
...
bool lexFile(const std::string& filename) {
  ...
  using token_type =
    lex::lexertl::token<iterator, boost::mpl::vector<std::string, int>, boost::mpl::true_>;
