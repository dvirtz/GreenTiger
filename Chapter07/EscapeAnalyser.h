...
private:
  ...
  template <typename T> void analyse(std::vector<T> &v);
  template <typename T> void analyse(boost::optional<T> &o);
  template <typename T>
...
template <typename T> void EscapeAnalyser::analyse(std::vector<T> &v) {
  std::for_each(v.begin(), v.end(),
                [this](auto &element) { this->analyse(element); });
}

template <typename T>
std::enable_if_t<boost::fusion::traits::is_sequence<T>::value>
  EscapeAnalyser::analyse(T &t) {
...
template <typename T> void EscapeAnalyser::analyse(boost::optional<T> &o) {
  if (o) {
    analyse(*o);
  }
}
