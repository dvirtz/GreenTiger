...
{
  auto res = boost::apply_visitor(*this, ast);
  //   if (res)
  //   {
  //     // get actual type
  //     auto pNameType = boost::get<NameType>(&res->m_type);
  //     while (pNameType)
  //     {
  //       res->m_type = pNameType->m_type;
  //       pNameType = boost::get<NameType>(&res->m_type);
  //     } while (pNameType);
  //   }
  return res;
}
...
Compiler::result_type Compiler::operator()(const ast::CallExpression & exp)
{
  ...
  for (size_t i = 0; i < argTypes.size(); ++i) {
    if (argTypes[i].m_name != paramTypes[i].m_name) {
...
Compiler::result_type Compiler::operator()(const ast::ArithmeticExpression & exp)
{
  ...
  if (!hasType<IntType>(*lhs)) {
    return m_errorHandler(id(exp.first), "Expression must be of type int");
  }
  ...
  for (const auto& opExp : exp.rest)
  {
    ...
    if (!hasType<IntType>(*rhs)) {
      return m_errorHandler(id(opExp.exp), "Expression must be of type int");
    }
...
Compiler::result_type Compiler::operator()(const ast::RecordExpression & exp)
{
  ...
  for (size_t i = 0; i < fieldTypes.size(); ++i) {
    if (fieldTypes[i].m_name != recordFields[i].m_type.m_name) {
...
Compiler::result_type Compiler::operator()(const ast::AssignExpression & exp)
{
  ...
  if (varType->m_type.m_name != expType->m_type.m_name) {
...
Compiler::result_type Compiler::operator()(const ast::IfExpression & exp)
{
  ...
  if (exp.elseExp)
  {
    ...
    if (thenExp->m_type.m_name != elseExp->m_type.m_name) {
...
Compiler::result_type Compiler::operator()(const ast::ArrayExpression & exp)
{
  ...
  if (arrayType->m_elementType.m_name != initExp->m_type.m_name) {
...
bool Compiler::addToEnv(const ast::FunctionDeclarations & decs)
{
  ...
  for (const auto& dec : decs) 
  {
    ...
    if (compiled->m_type.m_name != funcType->m_resultType.m_name) {
...
bool Compiler::addToEnv(const ast::VarDeclaration & dec)
{
  ...
  if (dec.type) {
    ...
    if (type->m_name != compiled->m_type.m_name & !hasType<NilType>(*compiled)) {
